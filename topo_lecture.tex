...
\[X^\diamond \to \Sigma X = \frac{X\times [0,1]}{X\times \set{0,1}\cup \set{x_0}\times [0,1]}\]
...
\begin{proof}
    ...
    \begin{remark}
        \(X\) \(k\)-connected \(\Lra\) \(\set{*}\to X\) \(k\)-connected \(\Lra\) \(X\to \set{*}\) is \((k+1)\)-connected.
    \end{remark}
\end{proof}
...
\begin{proposition}[11.12 for Lück]
    ...
    \begin{enumerate}
        ...
        \item \(\pi_k(X\times Y, X\vee Y, *)\) and \(\pi_k(X\wedge Y, *)\) are trivial for all \(0 \leq k\leq m+n+1\).
...
\begin{example}
    ...
    \(K(\IZ/2, 1) = \ir P^{\infty}\). For \(X\) a CW-complex, Also \(\mathrm{Pic}(X) \xleftarrow{\cong} [X, \ir P^\infty] \to H^1(X,\iz /2)\). \([\gamma\colon E\to X] \mapsto w_1(\gamma)\) is called the first Stiefel-Whitney class. So real line bundles are completely classified by their first Stiefel-Whitney class.
\end{example}
...
\begin{defi}{topological \(n\)-simplex}{}
    ...
    or equivalently
    \(\nabla^n\) is the convex hull of the standard basis vectors \(e_i = (0, \dots, 0, 1, 0, \dots, 0)\in \ir^{n+1}\) where the \(1\) is at the \(i\)-th place, starting with \(0\).
\end{defi}
...
\(\abs X\) is also the coend of the functor \(\Delta^{\text{op}}\times \Delta \to \mathbf{Top}, ([m], [n])\mapsto X_m\times \nabla^n\)\footnote{no idea what a coend is.}.
...
\textbf{Note.} Let \(X\) be a simplicial set, \(Y\subseteq X\) simplicial subset, \(Y_{m-1} = X_{m-1}\) or equally \(\sk^{m-1} Y = \sk^{m-1}X\). Then the following square commutes for all \(x \in X_m\)
\[\begin{tikzcd}
...
\begin{thm}{preferred CW-structure}{}
    ...
    \begin{enumerate}
        ...
        \item Suppose that for \(n > m\), all \(n\)-simplices of \(X\) are degenerate. Then \(\abs X\) is \(m\)-dimensional.
...
\newLecture{14.01.2025}

I slept very well, unfortunately that was during the lecture.

\newLecture{19.01.2025}

The aim for today is:



The adjunction unit \(\eta_X \colon X \to \cS\abs X\) is a homology isomorphism. We will later see, that the counit is a weak homotopy equivalence.

\begin{thm}{}{}
    Let \((K,L)\) be a pair of simplicial sets, \(L\subseteq K\) We have the following commutative square of chain complexes:
    \[\begin{tikzcd}
        {C_*(L,A)} \ar[r, hook] \ar[d, "{C_*(\eta_L, A)}"] & C_*(K,A) \ar[d, "{C_*(\eta_K, A)}"] \ar[r, two heads] & \frac{C_*(K,A)}{C_*(L,A)} \ar[d, "{\eta_{K,L}}"] \\
        {C_*(\cS\abs L, A)} \ar[r, hook] & {C_*(\cS\abs K, A)} \ar[r, two heads] & {\frac{C_*(\cS\abs K, A)}{C_*(\cS\abs L, A)}} \\
    \end{tikzcd}\]
    The chain map \(\eta_{K,L}\) is a quasi-isomorphism. In particular, the induced map on homology
    \[(\eta_{K,L})_*\colon H_k(K,L;A) \to H_*(\abs K, \abs L, A)\]
    is an isomorphism.
\end{thm}


\begin{proof}
    We proof this in 6 steps. The first 3 from last lecture
    \begin{enumerate}
        \item Compatibility with pushout squares.
        \item Let \(M\subseteq L\subseteq K\) be a triple of simplicial sets. If two of \(\eta_{K,M}, \eta_{K,L}, \eta_{L,M}\) are quasi-isomorphisms, then so is the third.
        \item \(\eta_{\Delta^m, \partial\Delta^m}\) is a quasi isomorphism for all \(m\geq 0\).
        \item Let \(\set{(K_i, L_i)}_{i\in I}\) be a family of pairs of simplicial sets. If \(\eta_{K_i, L_i}\) is a quasi-iso for all \(i \in I\), then so is \(\eta_{\coprod_{i\in I}K_i, \coprod_{i\in I} L_i}\). We see this by
        \[\bigoplus_{i\in I} C_*(K_i, A)/C_*(L_i, A) \xrightarrow{\bigoplus \eta_{K_i,L_i}} \bigoplus \frac{C_*(\cS\abs {K_i}, A)}{C_*(\cS\abs{L_i}), A}\]
        and using that
        \[\bigoplus_{i\in I} C_*(K_i, A)/ C_*(L_i, A) \cong \frac{C_*(\coprod_{i\in I} K_i, A)}{C_*(\coprod_{i\in I} L_i; A)}\]
        and using that \(\abs \cdot\) preserves corpoducts, but also \(\cS\) preserves coproducts by definition so
        \[\bigoplus\frac{C_*(\cS\abs{K_i}, A)}{C_*(\cS\abs{L_i}, A)} \cong \frac{C_*(\cS\abs{ \coprod K_i}, A)}{C_*(\cS\abs{\coprod L_i}, A)}\]
        \item The claim holds for \(((\sk^m K) \cup L, L)\) for all \(m \geq -1\) setting the \(m-1\)-skeleton to be empty. We show this by induction over \(m\). For \(m= -1\) this statement is clear.
        
        We set \(N\coloneq K_m^{\mathrm{nd}} \setminus L_m^{\mathrm{nd}}\) There is a pushout square of simplicial sets
        \[\begin{tikzcd}
            \coprod \partial\Delta^m \ar[r, hook] \ar[d] & \coprod_{N} \Delta^m \ar[d] \\
            (\mathrm{sk}^{m-1} K)\cup L \ar[r, hook] & (\sk^m K) \cup L \\
        \end{tikzcd}\]
        Now applying first claim 4 to see the top to be ok, Claim 1 to see the bottom to be ok too and then claim 2 together with the induction hypothesis for what we desired to be ok.
        \item \(\eta_{K,L}\) is a quasi-isomorphism in general. We fix a homological dimension \(n\). Then
        \[\begin{tikzcd}
            ((\sk^{n+1} K)\cup L, L) \ar[r, hook] & (K,L) \\
        \end{tikzcd}\]
        iduces an isomorphism on \(H_n(\_, \_; A)\) and similarly
        \[(\abs{(\sk^{n+1}K) \cup L}, \abs L) \to (\abs K, \abs L)\]
        induces an isomorphism on \(H_n(\_, \_; A)\).
    \end{enumerate}
\end{proof}


\begin{corollary}
    For every space \(Z\), the adjunction counit \(\varepsilon_Z\colon \abs{\cS(Z)} \to Z\) induces isomorphisms of all singular homology groups.
\end{corollary}

\begin{proof}
    The composite
    \[\cS(Z) \xrightarrow{\eta_{\cS(Z)}} \cS(\abs{\cS(Z)}) \xrightarrow{\cS(\varepsilon_Z)} \cS(Z)\]
    is the identity. So the composite
    \[H_*(Z;A) \xrightarrow{H_*(\eta_{\cS(Z)})} H_*(\abs{\cS(Z)}; A) \xrightarrow{H(\varepsilon_Z)} H_*(Z,A)\]
    is the identity and the first map is a isomorphism.
\end{proof}

\begin{proposition}
    Let \(X\) be a non-empty simplicial set. Suppose that:
    \begin{enumerate}
        \item for all simplices \(x, y \in X_0\), there is a \(w\in X_1\) s.t. \(d_1^*(w) = x, d_0^*(w) = y\).
        \item For all \(1\)-simplices \(u,v,w \in X_1\) such that \(d_0^*(u) = d_1^*(v), d_0^*(v) = d_0^*(w), d_1^*(u) = d_1^*(w)\), there is \(z \in X_2\) such that \(d_0^*(z) = v, d_1^*(z) = w, d_2^*(z) = u\)
    \end{enumerate}
    Then \(\abs X\) is simply connected.
\end{proposition}

\begin{proof}
    We choose a vertex \(x_0 \in X_0\). By \(i\) we can choose for every \(y \in X_0 \in X_0\setminus\set{x_0}\) a \(1\)-simplex \(s(y)\in X_1\), s.t. \(d_0^*(s(y)) = y, d_1^*(s(y) = x)\).
    We let \(T\) be the simplicial subset of \(X\) containing \(s(y)\) for all \(y \in X_0\), and \(x_0\). Then \(\abs T\) is a \(1\)-dimensional simplicial set, \(\abs T\) is a \(1\)-dimensional CW-subcomplex of \(\abs X\) and \(\abs T\) is contractible. Since \(\abs T\) is a CW-subcomplex of \(\abs X\), the inclusion has the HEP and \(\abs X \to \abs X /\abs T \cong \abs{X/T}\) is a homotopy equivalence.

    So we may show that \(\abs{X/T}\) is simply connected.
    \[\sk^1\abs{X/T} = \abs{\sk^1(X/T)} \cong \bigvee_{X_1^{\mathrm{nd}}\setminus\set{s(y)}_{y\neq x_0}} S^1\]
    \(X/T\) has a unique \(0\)-simplex, because \(T_0 = X_0\) so by van Kampens theorem \(\pi_1(\abs{\sk^1(X/T)}, x_0)\) is a free group with basis the class of loops indexed by \(X_1^{\mathrm{nd}}\setminus\set{s(y)}_{y\neq x_0}\). By cellular approximation we know
    \[\pi_1(\abs{\sk^1(X/T)}, x_0) \to \pi_1(\abs{X/T}, x_0)\]
    is surjective. So it suffices to show that all generators of the sort become trivial in the realization. Let \(v \in X_1^{\mathrm{nd}}\), \(v\neq s(y)\) for all \(y\). Then the conditions are perfectly satisfied to use 2. of the prerequistes. So we have a \(z \in X_2\), whose \(2\)-cell in the geometric realization parameterises a nullhomotopy.
\end{proof}

We could rephrase the conditions as maps from \(\partial\Delta^1\) extend to \(\Delta^1\) and similarly for \(\Delta^2\).

\begin{proposition}
    Let \(X\) be a simplicial set such that for all \(0 \leq k \leq n\), every morphism \(\partial \Delta^k \to X\) can be extended to \(\Delta^k\). Then \(\abs X\) is \((n-1)\) connected.
\end{proposition}

We don't need this so we don't proof this.

\begin{corollary}
    Let \(Z\) be a simply connected space.
    \begin{enumerate}
        \item The simplicial complex \(\cS(X)\) satisfies the conditions 1 and 2 of the previous proposition.
        \item \(\abs {\cS(Z)}\) is simply connected.
    \end{enumerate}
\end{corollary}

\begin{proof}
    I was to lazy to copy the proof of 1.
\end{proof}

\begin{thm}{}{}
    For every space \(Z\) the adjuntcion counit \(\varepsilon_Z\colon \abs{\cS(Z)}\to Z\) is a weak homotopy equivalence.
\end{thm}

\textbf{Note.} \(\varepsilon_Z\) is a natural CW-approximation!

\begin{proof}
    In 4 cases
    \begin{description}
        \item[Case 1] \(Z\) is simply connected and admits a CW-structure. Then by the previous proposition, \(\abs \cS(Z)\) is also simply connected. So \(\varepsilon_Z\colon \abs{\cS(Z)}\to Z\) is a homology isomorphism between simply connected CW-complexes, hence a homotopy equivalence.
        \item[Case 2] The spaece \(Z\) is path-connected and admits a CW-structure. There exists a universal cover \(p\colon \tilde Z\to Z\); in particular \(\tilde Z\) is simply connected and admits a CW-structure.
        
        \textbf{Claim.} \(\abs{\cS(\tilde Z)} \xrightarrow{\abs{\cS(p)}} \abs{\cS(Z)}\) is also a universal cover.
        
        Let \(G\) be the group of deck transformations of \(p\colon \tilde Z\to Z\). This is a free \(G\)-action on \(\tilde Z\), such that \(p\) descends to a homeomorphism \(\tilde Z/G\to Z\). By exercise 9.2 \(\cS(\tilde Z)/G \xrightarrow{\cong} \cS(\tilde Z/G) \cong \cS(Z)\). Since the \(G\)-action on \(\cS(Z)\) is dimensionwise free, also the action on \(\abs{\cS(\tilde Z)}\) is free. So
        \[\abs {\cS(\tilde Z)} \to \abs{\cS(\tilde Z)}/G \cong \abs{\cS(\tilde Z)/G} \cong \abs{\cS(\tilde Z/G)} \cong \abs{\cS(Z)}\]

        So altogether we get a commutative diagram
        \[\begin{tikzcd}
            \abs{\cS(\tilde Z)}\ar[d]\ar[r, "\varepsilon_{\tilde Z}"] & \tilde Z \ar[d, "p"] \\
            \abs{\cS(Z)} \ar[r, "\varepsilon_Z"] & Z \\
        \end{tikzcd}\]
        By case 1 \(\varepsilon_{\tilde Z}\) is a homotopy equivalence. So \(\varepsilon_{\tilde Z}\) induces isomorphisms on \(\pi_n\) for \(n \geq 2\). 
    \end{description}
\end{proof}

\newLecture{21.01.2026}

I seem to not be able to wake up well on Wednesdays.

\newLecture{26.01.2026}

The Aim for today is to construct equivalences of categories.
\[\begin{tikzcd}
    \mathbf{Top}[\text{w.eq}^{-1}] \ar[d, bend left, "\cS"] & \mathrm{Ho}(\mathrm{CW}) \ar[l, "\incl_*"] \\
    \mathbf{sset}[\text{w.eq}^{-1}] \ar[u, bend left, "\abs\cdot"] \ar[ru, "\abs\cdot"] \\
\end{tikzcd}\]

\begin{defi}{Weak equivalence}{}
    A morphism \(f\colon X \to Y\)  of simplicial sets is a \emph{weak equivalence}, if
    \[\abs f\colon \abs X\to \abs Y\]
    is a homotopy equivalence
\end{defi}

This is a weaker notion than homotopy equivalence.

\begin{proposition}
    For every simplicial set \(X\), the adjunction unit \(\eta_X\colon X \to \cS\abs X\) is a weak equivalence of simplicial sets.
\end{proposition}

\begin{construction}[localization of simplicial sets at weak equivalences]
    We define a category \(\mathbf{sset}[\mathrm{w.eq}^{-1}]\) with objects the same as \(\mathbf{sset}\).
    \[\Hom_{\mathbf{sset}[\mathrm{w.eq}^{-1}]}(X,Y) \coloneq \Hom_{\mathbf{Top}}(\abs X, \abs Y)/\text{Homotopy } = \mathbf{Ho}(\mathrm{CW})(\abs X, \abs Y)\]
    This comes with a functor
    \[\gamma\colon \mathbf{sset} \to \mathbf{sset}[\mathrm{w.eq}^{-1}]\]
    such that \(\gamma = \id\) on objects and \(\gamma(f) = [\abs f] = \text{ homotopy class of } \abs f\colon \abs X \to \abs Y\)
\end{construction}

\begin{proposition}[\enquote{calculus of fractions}]
    Let \(X,Y\) be simplicial sets and \(\alpha\colon \abs X \to \abs Y\) a continuous map.
    \begin{enumerate}
        \item The following square of continuous maps
        \[\begin{tikzcd}
            \abs X \ar[r, "\abs{\eta_X}"]\ar[d, "\alpha"] & \abs{\cS\abs X} \ar[d, "\abs{\cS(\alpha)}"] \\
            \abs Y \ar[r, "\abs{\eta_Y}"] & \abs{\cS\abs Y} \\
        \end{tikzcd}\]
        commutes up to homotopy.
        \item The relation
        \[\begin{split}
            [\alpha] &= \gamma(\eta_Y)^{-1} \circ \gamma(\cS(\alpha)\circ \eta_X)
        \end{split}\]
        holds.
    \end{enumerate}
\end{proposition}
...
\begin{proof}
    \begin{enumerate}
        \item \[\begin{tikzcd}
            \abs X \ar[r, "\abs{\eta_X}"] \ar[d, "\alpha"] & \abs{\cS\abs X} \ar[d, "\abs{\cS(\alpha)}"] \ar[r, "\varepsilon_{\abs X}"] & \abs X \ar[d, "\alpha"] \\
            \abs Y \ar[r, "\abs{\eta_Y}"] & \abs{\cS \abs Y} \ar[r, "\varepsilon_{\abs Y}"] & \abs Y \\
        \end{tikzcd}\]
        And the outside of the diagram commutes. \(\varepsilon_{\abs Y}\) is a weak equivalence between CW-complexes, hence a homotopy equivalence so the left square commutes up to homotopy.
        \item Because the left square commutes up to homotopy, it is ?? commutatvie square in \(\mathbf{sset}[\mathrm{w.eq}^{-1}]\)
        and somehow that helps.
    \end{enumerate}
\end{proof}

\begin{thm}{localization of simplicial sets}{}
    The functor \(\gamma\colon \mathbf{sset}\to \mathbf{sset}[\mathrm{w.eq}^{-1}]\) is a localisation of simplicial sets at weak equivalences.
\end{thm}

\begin{proof}
    The functor \(\abs \cdot \colon \mathbf{sset}\to \mathbf{Top}\) takes weak equivalences to homotopy equivalences, which are isomorphisms in \(\mathrm{Ho}(\mathrm{Top})\) hence also in \(\mathbf{sset}[w.eq^{-1}]\). So \(\gamma\) inverts weak equivalences.

    Let \(F\colon \mathbf{sset}\to \cD\) be any functor that inverts weak equivalences.
    \begin{description}
        \item[Uniqueness] Let \(G\colon \mathbf{sset}[\mathrm{w.eq.}^{-1}]\to D\) be a functor such that \(G \circ \gamma = F\). Since \(\gamma\) is the identity on objects, we have \(G = F\) on objects. For uniqueness on morphisms. apply \(G\) to the fraction realation.
        \[\begin{split}
            G[\alpha] &= G(\gamma(\eta_Y)^{-1}\circ \gamma(\cS(\alpha)\circ \varepsilon_X)) \\
            &= G(\gamma(\eta_Y))^{-1} \circ G(\gamma(\cS(\alpha)\circ \varepsilon_X)) \\
            &= F(\eta_Y)^{-1} \circ F(\cS(\alpha) \circ \varepsilon_X) \\
        \end{split}\]
        \item[Existence] Given \(F\colon \mathbf{sset} \to \cD\), inverting weak equivalences, we define \(G\) on objects as \(F\) and on morphisms Let \(\alpha\colon \abs X \to \abs Y\) be any continuos map, we define \(([\alpha]\colon X \to Y \text{ in } \mathbf{sset}[\mathrm{w.eq}^{-1}])\)
        \[G[\alpha] = F(\eta_Y)^{-1} \circ F(\cS(\alpha)\circ \varepsilon_X)\]
        We check well definedness. We show first that \(F\) takes simplicially homotopic morphisms to the same morphism.
        Let \(H\colon X \times \Delta^1\to Y\) be a simplicial homotopy. The projection \(X\times \Delta^1\to X\) realizes to a homotopy equivalence
        \[\begin{tikzcd}
            \abs{X\times \Delta^1} \ar[d, phantom, sloped, "\cong"]\ar[r, "\abs p"] & \abs X \\
            \abs X \times \abs {\Delta^1} \ar[ru, "pr"] \\
        \end{tikzcd}\]
        so \(p\) is a equivalence hence \(F(p)\colon F(X\times \Delta^1)\to F(X)\) is an isomorphism. Moreover \(p\circ i_0 = \id_X = p\circ i_1\). Since \(F(p)\) is an isomorphism, this yields \(F(i_0) = F(i_1)\). Then \(F(f) = F(g)\). So \(F\) takes simpilcially homotopic morphisms of simplicial sets to the same morphism in \(D\).

        Now let \(\alpha\colon \alpha' \colon \abs X\to \abs Y\) be homotopic continuous maps. Then \(\cS(\alpha), \cS(\alpha')\) are simplicially homotopic. So \(F(\cS(\alpha)) = F(\cS(\alpha'))\). So \(G[\alpha]\) is well defined, i.e. only depends on the homotopy class of \(\alpha\).

        We still need to check that \(G\) is a functor. Let \(\alpha\colon \abs X \to \abs Y, \beta\colon \abs Y\to \abs Z\) be continuous maps, representing morphisms \([\alpha]\colon X\to Y, \beta \colon Y\to Z\) in \(\mathbf{sset}[\mathrm{w.eq.}^{-1}]\)
        \[\begin{split}
            G[\beta]\circ G[\alpha] &= F(\eta_Z)^{-1} \circ F(\cS(\beta)\circ \eta_Y) \circ F(\eta_Y)^{-1} \circ F(\cS(\alpha)\circ \eta_X) \\
            &= F(\eta_Z)^{-1} \circ F(\cS(\beta))\circ F(\eta_Y) \circ F(\eta_Y)^{-1} \circ F(\cS(\alpha)\circ \eta_X) \\
            &= F(\eta_Z)^{-1} \circ F(\cS(\beta\circ \alpha)\circ \eta_X) \\
        \end{split}\]
        Checking the identity is easy. We need to check \(G\circ \eta = F\). Let \(f\colon X \to Y\) be a morphism of simplicial sets. Naturality of \(\eta\colon \id_{\mathbf{sset}} \to \cS \circ \abs \cdot\) gives a long equation that helps.
    \end{description}
\end{proof}

The equivalences between \(\mathbf{sset}[\mathrm{w.eq.}^{-1}]\) and \(\mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\).

The functor \(\abs\cdot\colon \mathbf{sset}\to \mathbf{Top}\) takes values in \(\mathbf{Top}_{\mathrm{CW}}\) the full subcategory of spaces admitting a CW-structure. The composite functor
\[\mathbf{sset}\xrightarrow{\abs \cdot} \mathbf{Top}_{\mathrm{CW}} \xrightarrow{\text{proj.}} \mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\]
inverts weak equivalences. The universal property of the localization \(\gamma\colon \mathbf{sset} \to \mathbf{sset}[\mathrm{w.eq.}^{-1}]\) provieds a unique functor \(\Psi\colon \mathbf{sset}[\mathrm{w.eq.}^{-1e}] \to \mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\).

\begin{thm}{}{}
    The functor \(\Psi\colon \mathbf{sset}[\mathrm{w.eq}^{-1}] \to \mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\) is an equivalence of categories.
\end{thm}

\begin{proof}
    Unraveling all definitions shows that on objects \(\Psi(X) = \abs X\) and on morphisms
    \[\Hom_{\mathbf{sset}[\mathrm{w.eq.}^{ -1}]}(X, Y) = \mathbf{Top}(\abs X, \abs Y)/\text{homotopy} \xrightarrow{\id} \mathbf{Top}(\abs X, \abs Y)/\text{homotopy}\]
    so \(\Psi\) is fully faithful.

    For essential surjectivity of \(\Psi\) let \(K\) be a space that admits a CW-structure. Then
    \[\varepsilon_K\colon \abs{\cS(K)} \to K\]
    is a weak homotopy equivalence between CW-complexes, hence a homotopy equivalence. So \([\varepsilon_X]\colon \abs{\cS(K)} \xrightarrow{\cong} K\) is an isomorphism in \(\mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\). So every object of \(\mathrm{Ho}(\mathbf{Top}_{\mathrm{CW}})\) is isomorphic to an object in the image of \(\Psi\)
\end{proof}

Now we do this for the second equivalence.

\begin{construction}[Localization of spaces at weak equivalences.]
    we define the category \(\mathbf{Top}[\mathrm{w.eq.}^{-1}]\) on Objects just as objects of \(\mathbf{Top}\).
    \[\Hom_{\mathbf{Top}[\mathrm{w.eq.}^{-1}]}(A,B) \coloneq \Hom_{\mathbf{Top}}(\abs{\cS(A)}, \abs{\cS(B)})/\text{homotopy}\]

    note that \(\abs{\cS(A)}\xrightarrow{\varepsilon_A} A\) is a functorial CW-approximation. We define a functor \(\gamma\colon \mathbf{Top} \to \mathbf{Top}[\mathrm{w.eq.}^{-1}]\) as the identity on objects and on morphisms by
    \[\gamma(f\colon A\to B) = [\abs{\cS(f)}]\]
\end{construction}

\begin{proposition}
    Let \(A\) and \(B\) be topological spaces, Let \(\alpha\colon \abs{\cS(A)}\to \abs{\cS(B)}\) be a continuous map. Then
    \[[a] = \gamma(\varepsilon_B \circ \alpha) \circ \gamma(\varepsilon_A)^{-1}\]
    in \(\Hom_{\mathbf{Top}[\mathrm{w.eq.}^{-1}]}(A,B)\)
\end{proposition}

\begin{proof}
    We will show that the diagram of spaces and continuous maps commutes up to homotopy.
    \[\begin{tikzcd}
        \abs{\cS\abs{\cS(A)}} \ar[r, "\abs{\cS(\varepsilon_A)}"]\ar[d, "\abs{\cS(\alpha)}"] & \abs \cS(A) \ar[d, "\alpha"] \\
        \abs{\cS\abs{\cS(B)}} \ar[r, "\abs{\cS(\varepsilon_B)}"] & \abs{\cS(B)} \\
    \end{tikzcd}\]
    We show this by extending the diagram on the left. We then use triangle identities and naturality of \(\eta\).

    We take
    \[\begin{split}
        \abs{\cS(\varepsilon_B)}\circ \abs \cS(\alpha) \circ \abs{\eta_{\cS(A)}} &\sim \abs{\cS(\varepsilon_B)} \circ \abs {\eta_{\cS(B)}} \circ \alpha \\
        &= ?? = \alpha \\
        &= \alpha\circ \abs{\cS(\varepsilon_A)} \circ \abs{\eta_{\cS(A)}}
    \end{split}\]
    Since \(\abs{\eta_{\cS(A)}}\) is a homotopy equivalence, we can cancel it up to homotopy, and conclude that the square commutes up to homotopy.

    And then he somehow concluded the proof.
\end{proof}

\newLecture{28.01.2026}

\begin{thm}{Localization of topological spaces}{}
    The functor \(\gamma\colon \mathbf{Top} \to \mathbf{Top}\weq\) is a localization at the class of weak equivalences.
\end{thm}

\begin{proof}
    The composite functor \(\abs\cdot\circ \cS \colon \topc \to \topc\) takes weak equivalences to homotopy equivalences, so \(\gamma\) inverts weak equivalences.

    Let \(F\colon \topc \to \cD\) be any functor that inverts weak equivalences.
    \begin{description}
        \item[Uniqueness] Let \(G\colon \topc\weq\to \cD\) be a functor such that \(G\circ \gamma = F\). Then on objects, \(G(A) F G(\gamma(A)) = F(A)\). On morphisms for
        \[[\alpha]\in \Hom_{\topc\weq}(A,B), \alpha\colon \abs{\cS(A)}\to \abs{\cS(B)}\]
        we have
        \[\begin{split}
            G[\alpha] &= G(\gamma(\varepsilon_B \circ \alpha)) \circ \gamma(\varepsilon_A)^{-1} \\
            &= G(\gamma(\varepsilon_B\circ \alpha)) \circ G(\gamma(\varepsilon_A))^{-1} = F(\varepsilon_B\circ \alpha) \circ F(\varepsilon_A)^{-1} \\
        \end{split}\]
        \item[Existence] Given \(F\colon \topc \to \cD\), \(F\) inverting weak equivalences, we define \(G\colon \topc\weq \to \cD\) as \(F\) on objects and
        \[G[\alpha] = F(\varepsilon_B \circ \alpha)\circ F(\varepsilon_A)^{-1}\]
        on morphisms. Well definedness uses that
        \begin{itemize}
            \item \(F\) takes \(p\colon A \times [0,1] \to A\) to an isomorphism
            \item \(F(i_0) = F(i_1)\) for the 2 inclusions \(A \to A\times [0,1]\).
            \item \(F(f) = F(g)\) for homotopic \(f,g\)
        \end{itemize}
        To show that \(G\) is a functor we have \(G(\id_X) = \id_{G(X)}\). Let \(\alpha\colon \abs{\cS(A)}\to \abs{\cS(B)}\), \(\beta\colon \abs{\cS(B)}\to \abs{\cS(C)}\) be continuous maps.
        \[\begin{split}
            G[\beta]\circ G[\alpha] &= F(\varepsilon_C) \circ F(\beta) \circ F(\varepsilon_B)^{-1} \circ F(\varepsilon_B) \circ F(\alpha) \circ F(\varepsilon_A)^{-1} \\
            &= F(\varepsilon_C) \circ F(\beta\circ \alpha) \circ F(\varepsilon_A)^{-1} = G[\beta\circ \alpha]\\
        \end{split}\]
    \end{description}
\end{proof}

We will now show the equivalence of categories \(\sset\weq \cong \topc\weq\) by showing the adjoint functor pair \(\abs\cdot, \cS\) descends to an adjoint equivalence.

The realization functor \(\abs\cdot\colon \sset \to \topc\) preserves weak equivalences, so the composite \(\sset \to \topc \to \topc\weq\) inverts weak equivalences. So there is a unique functor
\[\alpha\colon \sset\weq \to \topc\weq\]
so that \(\alpha\circ \gamma = \gamma\circ \abs\cdot\). Similarly we get a unique functor
\[\beta\colon \topc \weq \to \sset\weq\]
such that \(\beta\circ \gamma = \gamma\circ \cS\). we have
\[\begin{tikzcd}
    \sset \ar[r, "\abs\cdot"] \ar[d, "\gamma"] & \topc \ar[d, "\gamma"] \ar[r, "\cS"] & \sset \ar[d, "\gamma"] \\
    \sset\weq \ar[r, "\alpha"] & \topc\weq \ar[r, "\beta"] & \sset\weq \\
\end{tikzcd}\]

\begin{thm}{Equivalence of categories}{}
    The composite functor \(\beta\circ \alpha \colon \sset\weq \to \sset\weq\) and \(\alpha\circ \beta\colon \topc\weq \to \topc\weq\) are naturally isomorphic to the respective identity functions.

    In particular \(\alpha, \beta\) are equivalences of categories.
\end{thm}

\begin{proof}
    We give the proof for \(\beta\circ \alpha\), the other one being completely analogous. We compose the adjunction unit \(\eta\colon \id_{\sset}\implies \cS\circ \abs\cdot\)\footnote{natural transformations now have the double arrows.} with localisation functor \(\gamma\colon \sset \to \sset\weq\). We obtain a natural transformation \(\gamma\circ \eta\colon \gamma \implies \gamma\circ \cS\circ \abs\cdot = \beta\circ \gamma \circ \abs\cdot = \beta\circ \alpha\circ \gamma\colon \sset \to \sset\weq\).
    This is a natural transformation between weak equivalence inverting functors. By the natural-transformation part of localization property, there is a uniqe natural transformation
    \[\tau\colon \id_{\sset\weq}\implies \beta\circ \alpha\]
    such that \(\tau\circ \gamma = \gamma\circ \eta\). Then for all simplicial sets \(X\), \(\tau_X = \tau_{\gamma(X)} = \gamma(\eta_X)\) is an isomorphism, because \(\eta_X\colon X \to \abs{\cS(X)}\) is a weak equivalence of simplicial sets. So \(\tau\colon \id_{\sset\weq} \to \beta\circ \alpha\) is a natural isomorphism.
\end{proof}

We will see that we have equivalences of categories
\[\begin{tikzcd}
    \Ho(\topc_{\mathrm{CW}}) \ar[d, "\incl_*"] & \sset\weq\ar[l, "\abs\cdot", "\cong"'] \\
    \topc\weq \ar[r, "\cS"] & \Ho(\sset_{\mathrm{Kan}}) \\
\end{tikzcd}\]

We want to define what a Kan-complex is. Let \(n\geq 1\), \(0\leq i\leq n\). The \(i\)-th horn
\[\Lambda_i^n\]
is the simplicial subset of \(\Delta^n\) generated by \(d_0, \dots, d_{i-1}, d_{i+1},\dots, d_n\). We have \(\Lambda_i^n \subseteq \partial\Delta^n \subseteq \Delta^n\). Schwede draws some horns. 

\begin{defi}{Kan-Complex}{}
    A \emph{Kan-Complex} is a simplicial set with the following property: For all \(n\geq 1, 0 \leq i\leq n\), all morphisms \(f\colon \Lambda_i^n\to X\) there is a morphism \(g\colon \Delta^n\to X\) such that \(g \rvert_{\Lambda_i^n} = f\).
\end{defi}

There is also a more \enquote{down-to-earth}-definition of Kan-complexes.

\begin{proposition}
    The following is a co-equalizer diagram in the category of simplicial sets:
    \[\coprod_{0\leq i < j\leq n-1}\Delta^{n-2} \xrightarrow{\alpha,\beta}\coprod_{0\leq l\leq n, l\neq k} \Delta^{n-1} \xrightarrow{\gamma}\Lambda_k^n\]
    where \(\gamma\) is \(d_l\colon \Delta^{n-1} \to \Delta^n\) on the \(l\)-th summand, \(\alpha\) is \(d_{j-1}\colon \Delta^{n-2}\to \Delta^{n-1}\) into the \(l = i\)-th summand, \(\beta\) is \(d_i \colon \Delta^{n-1}\to \Delta^{n-1}\) into the \(l = j\)-th summand.
    
    The condition \(\gamma\circ \alpha = \gamma\circ \beta\) is equal to
    \[d_i \circ d_{j-1} = j_j \circ d_i\]
    for all \(i \leq j\).
\end{proposition}

\begin{corollary}
    A simplicial set \(X\) is a Kan-complex, if and only if the following holds:
    For all \(n \geq 1\), for all \(0\leq k \leq n\), for all \((x_0, \dots, x_{k-1}, x_{k+1}, \dots, x_n) \in X_{n-1}\), such that
    \[d_i^*(x_j) = d_{j-1}^*(x_i)\]
    for all \(0 \leq i < j \leq n\) there is an \(z \in X_n\), such that \(d_i^*(z) = x_i\) for all \(i \in \set{0, \dots, k-1, k+1, \dots, n}\).
\end{corollary}

The composite \(\abs{\Lambda_i^n} \xrightarrow{\abs{\incl}} \abs{\Delta^n} \cong \nabla^n\) is a closed embedding with image the \enquote{topological horn}, i.e. the union of the faces \((d_j)_*(\nabla^{n-1})\) for \(j= 0, \dots, i-1, i+1, \dots, n\). This is a deformation retraction.

\begin{corollary}
    For every space \(A\), the simplicial set \(\cS(A)\) is a Kan-complex.
\end{corollary}

\begin{proof}
    Let \(f\colon \Lambda_i^n \to \cS(A)\) be a morphism of simplicial sets. Let \(f^\flat\colon \abs{\Lambda_i^n} \to A\) be its adjoint. We can extend this by choosing a retraction \(r\colon \abs{\Delta^n}\to \abs{\Lambda_i^n}\) and setting \(g\colon f^\flat \circ r\). We take the adjoint \(g^\sharp\colon \Delta^n \to \cS(A)\) of \(g\). Then \(g^\sharp\) extends \(f\).
\end{proof}

Now Schwede just wanted to make some more remarks.

\begin{itemize}
    \item for morphisms into a Kan-complex \enquote{simplicial homotopy} is already symmetric and transitive, hence an equivalence relation.
    \item If \(X\) is any simplicial set and \(Y\) is a Kan-complex, then the map
    \[[X,Y]_{\sset} = \Hom_\sset(X,Y)/\text{homotopy} \to [\abs X, \abs Y]\]
    is bijective. We can interpret this as \enquote{simplicial approximation}.
    \item For every CW-complex \(A\) and all spaces \(B\),
    \[\cS\colon [A,B] \to [\cS(A), \cS(B)]_\sset\]
    is also bijective.
    \item Kan-complexes yield an intrinsic characterisation of weak equivalences of simplicial sets: a morphism \(f\colon X \to Y\) of simplicial sets is a weak equivalence, iff for all Kan-complexes \(Z\), the map
    \[[Y,Z]_\sset \xrightarrow{f^*} [X,Z]_\sset\]
    is bijective.
\end{itemize}

\begin{defi}{\(\infty\)-categories}{}
    A \emph{quasi-category} is a simplicial set in which all inner horns can be filled. For all \(n \geq 2\) \(0 < i < n\)\footnote{here \(<\) instead of \(\leq\) for inner horns} every morphism \(f\colon \Lambda_i^n \to X\) can be extended to \(\Delta^n\)
\end{defi}

A quasi-category is one model of an \(\infty\)-category.


\enquote{This is the beginning of a beautiful story.}
