...
\(\cA^*(A)\) acts tautologically on the \(A\)-cohomology on any space, by \(\tau\in \mathrm{Stab}(A,A,n)\), \(x\in H^m(X,A)\).
...
\begin{proof}
    \begin{enumerate}
        ...
        Thus \(P_p(\iota_n\times \iota_m) = P_p((c\circ \pi)^*(\iota_{n+m})) = \Delta^*_{K(n)\times K(m)}(:_p(c\circ \pi)^*(\iota_{n+m,p})) = \Delta_{K(n)\times K(m)}(\Delta^*(\iota_{n,p}\times \iota_{m,p})) = \Delta^*((\Delta_{K(n)}\times \Delta_{K(m)})(\iota_{n,p}\times \iota_{m,p}))= \Delta^*(P_p(\iota_n)\times P_p(\iota_m))\)
    \end{enumerate}
\end{proof}
