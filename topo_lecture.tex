...
Was ill, might add in later. \iffalse This is from Tiens (thank him for it) lecture notes

Reminder:

Let \(f \colon X \to Y\) be a continuous map between simply connected CW\nbhyph complexes.
Then the following are equivalent:
\begin{enumerate}
    \item \(f\)~is a weak homotopy equivalence.
    \item \(f\)~is a homotopy equivalence (Whitehead theorem).
    \item \(f_* \colon H_i(X; \Z) \to H_i(Y; \Z)\) is an isomorphism for \(i \ge 2\).
\end{enumerate}

Let \(f \colon X \to Y\) be a continuous map between path connected CW\nbhyph complexes.
Let \(\tilde{f} \colon \tilde{X} \to \tilde{Y}\) be a lift to universal covers.
Then the following are equivalent:
\begin{enumerate}
    \item \(f\)~is a homotopy equivalence.
    \item \(\tilde{f}_* \colon H_i(\tilde{X}; \Z) \to H_i(\tilde{Y}; \Z)\) is an isomorphism for \(i \ge 2\).
\end{enumerate}

Relative Hurewicz theorem with~\(\pi_1\): Let \((X,A)\) be a space pair with \(X\)~and~\(A\) path connected.
Suppose that \(\incl_* \colon \pi_1(A,a) \to \pi_1(X,a)\) is an isomorphism and that \((X,A)\) is \((n+1)\)\bhyph connected.
Then \(H_i(X,A; \Z) = 0\) for all \(0 \le i \le n-1\), and the modified Hurewicz map
\begin{equation*}
    h^{\dag} \colon \pi_n(X,A,a)^{\dag} \to H_n(X,A; \Z)
\end{equation*}
...
CW\nbhyph approximation: Let \((Y,A)\) be a space pair with \(A\) Hausdorff.
Then there is a \(n\)\nbhyph CW\nbhyph model \((Z,f)\) for \((Y,A)\) (\(n \ge 0\)), i.\,e.:
\begin{itemize}
    \item \((Z,A)\) is a relative CW\nbhyph complex with no relative cells in dimension \(0 \le i \le n\) (we can make it an absolute CW\nbhyph complex by making the attaching maps cellular before attaching);
...
\begin{theorem}[Lück Thm.~6.8]
    Let \(Y\) be any space.
    \begin{enumerate}
        \item There is a \defemph{CW\nbhyph approximation}, i.\,e., a pair \((X,f)\) consisting of a CW\nbhyph complex~\(X\) and a weak homotopy equivalence \(f \colon X \iso Y\).
        \item Let \((X,f)\) and \((X',f')\) be two CW\nbhyph approximations of~\(Y\).
...
\begin{proof}\leavevmode
    \begin{enumerate}
        \item If \(Y = \varnothing\), take \(X = \varnothing\).
            ...
            Let \((Z_C, f_C)\) be a \(0\)\nbhyph CW\nbhyph approximation of~\(Y_C\), i.\,e., the path component of~\(C\), so \(f_C \colon Z_C \to Y_C\) is a weak homotopy equivalence.
            Then set \(X = \coprod_{C \in \pi_0(Y)} Z_C \to[\coprod f_C] Y\), which is a CW\nbhyph approximation.
        \item Recall that if \(X\) is a CW\nbhyph complex and if \(f \colon Y \to Z\) is a weak homotopy equivalence, then \([X,f] \colon [X,Y] \to [X,Z]\) is bijective.

            Since \(f' \colon X' \to Y\) is a weak homotopy equivalence and since \(X\) is a CW\nbhyph complex, \([X,f'] \colon [X,X'] \iso [X,Y]\) is bijective.
...
Let \((Z,f)\) be an \(n\)\nbhyph CW\nbhyph model of \((CA,A)\) where \(CA = A \times [0,1] / A \times 1\) is the cone.
Then, for \(n \ge 1\),
\begin{equation*}
...
Also \((Z,A)\) is a relative CW\nbhyph complex with relative cells of dimension \(n+1\) or larger.
...
\begin{itemize}
    \item Let \(n \ge 1\), and let \(A\) be a group, abelian if \(n \ge 2\).
        Then there exists a path connected CW\nbhyph complex \(X = K(X,n)\) such that
        \begin{equation*}
            \pi_i(X,x) \cong
            \begin{cases}
                A & \text{if } i = n, \\
    ...
    \item Representability of cohomology: for \(Y\) a CW\nbhyph complex and \(A\) abelian, we have \(H^n(Y,A) \cong [Y, K(A,n)]\).
\end{itemize}
...
\begin{definition}
    ...
    An \defemph{Eilenberg\bhyph MacLane space} of type \((A,n)\) is a pair \((X, \varphi)\) consisting of a based path connected space \((X,x)\) and an isomorphism \(\varphi \colon \pi_n(X,x) \iso A\) such that \(\pi_i(X,x) = 0\) for all \(i \ge 1\) and \(i \ne n\).
    Shorthand notation: \enquote{\(X\)~is a~\(K(A,n)\)}.
\end{definition}

(In full generality, Eilenberg\bhyph MacLane spaces are very abstract\@.)

\begin{example}
    \(S^1\) is a \(K(\Z,1)\) since \(\pi_1(S^1,z) \cong \Z\) and \(\pi_i(S^1,z) = 0\) for all \(i \ge 2\) by covering space theory, because the universal cover \(\exp \colon \R \to S^1\) of~\(S^1\) has a contractible total space.
    ...
    Hence:
    \begin{itemize}
        \item \(\RP^0 = K(\Z/2, 1)\) with universal cover \(S^{\infty} \simeq *\).
        \item \(S^1 \times \dots \times S^1 = K(\Z^m, 1)\) (\(m\)~times) with universal cover~\(\R^m\).
        \item Klein bottle (insert diagram of a square with arrows on the edges that indicate how the Klein bottle is glued) is a \(K(\Z \rtimes \Z, 1)\) with universal cover~\(\R^2\).
            Here, \(\rtimes\)~is a semidirect product with \(\Z\)\nbhyph action on~\(\Z\) by sign.
    \end{itemize}
\end{example}

\begin{example}
    \(\CP^{\infty}\)~is a \(K(\Z,2)\) because of the locally trivial fibre bundle \(S(\C^{\infty}) \to \CP^{\infty}\), \(x \mapsto \C x\) with fibre~\(S^1\). 
    This is a Serre fibration, so the long exact sequence of homotopy groups gives
    \begin{equation*}
        \set{0} = \pi_i(S(\C^{\infty}), *) \to \pi_i(\CP^{\infty}, *) \iso[\partial] \pi_{i-1}(S^1, x) \to \pi_{i-1}(S(\C^{\infty}), *) = \set{0}.
    \end{equation*}
    Thus,
    \begin{equation*}
        \pi_i(\CP^{\infty}, *) \cong
        ...
        \end{cases}
    \end{equation*}
    (Assume that Eilenberg\bhyph MacLane spaces are CW\nbhyph complexes is not a huge loss of generality because of CW\nbhyph approximation\@.)
\end{example}
...
Start with the following \(2\)\nbhyph dimensional CW\nbhyph complex
\begin{equation*}
    CG \coloneq
    \pn*{\bigvee_{g \in G} S_g^1} \cup \bigcup_{(h,k) \in G^2} D_k^2
\end{equation*}
The \(2\)\nbhyph cells are attached as follows: let \(i_g \colon S^1 \to \bigvee_{g \in G} S_g^1\) denote the inclusion of the \(g\)th 
...
By covering space theory\slash Seifert\bhyph van~Kampen, we know that \(\pi_1(\bigvee_{g \in G} S_g^1, *)\) is a free group generated by the elements \([i_g]\).
...
By Seifert\bhyph van~Kampen, attaching a \(2\)\nbhyph cell to a path connected space precisely kills the normal subgroup generated by the class of the attaching map.
Thus,
\begin{equation*}
    \pi_1(CG,*)
    = \frac{\text{free group on elements of } G}{\ag{[h] [k] = [hk]}}
    \cong G.
\end{equation*}
...
We set \(X_n = \set{x} \bigcup_{J \times S^{n-1}} J \times D^n \cong \bigvee_J S^n\) as an \(n\)\nbhyph dimensional CW\nbhyph complex with one \(0\)\nbhyph cell and no cells in dimension \(1 \le i \le n-1\).
So, \(X_n\)~is \((n-1)\)\bhyph connected by cellular approximation.
Then \(\Z[J] \iso \tilde{H}_n(X_n; \Z)\), \(j \mapsto (i_j)_* [D^n, S^{n-1}]\) where \([D^n, S^{n-1}] \in H_n(D^n, S^{n-1}; \Z)\) is a chosen generator and \(i_j .: D^n \to \set{x} \cup_{J \times S^{n-1}} J \times D^n\) is the characteristic map of the \(j\)th \(n\)\nbhyph cell.
By the Hurewicz theorem, \(h \colon \pi_n(X_n, *) \to H_n(X_n; \Z) \cong \Z[J]\) is an isomorphism.
...
We define \(X_{n+1}\) by attaching \((n+1)\)\bhyph cells along all the \(\alpha_i\)'s: \(X_{n+1} = X_n \cup_{I \times S^n, \alpha_i} I \times D^{n+1}\).
The cellular chain complex of~\(X_n\) gives
\begin{equation*}
    \begin{tikzcd}
        H_{n+1}(X_{n+1}, X_n; \Z) \ar[r]
        & H_n(X_n; \Z) \ar[r]
        & H_n^{\mathrm{cell}}(X_{n+1}; \Z) \ar[r]
        & 0 \\
        \Z[I] \ar[u, "\sim"' sloped, "{i \mapsto (\beta_i)_* [D^{n+1}, S^n]}"] \ar[r, "d"]
        & \Z[J] \ar[r] \ar[u, "\sim"' sloped]
        & A \ar[r] \ar[u, dashed, "\sim"' sloped, "\exists!"]
        & 0
    \end{tikzcd}
\end{equation*}
with exact rows where \(\beta_i \colon D^{n+1} \to X_{n+1}\) is the characteristic map for the \(i\)th \((n-1)\)\bhyph cell.
The left square commutes by construction, so we obtain the dashed map.
Hence we have an isomorphism \(A \cong H_n(X_{n+1}; \Z)\).
Since \(X_{n+1}\) is \((n-1)\)\bhyph connected, the Hurewicz map is an isomorphism \(h \colon \pi_n(X_{n+1}, *) \iso H_n(X_{n+1}; \Z) \cong A\).
By killing the homotopy groups~\(\pi_i\) for \(i \ge n+1\) from~\(X_{n+1}\) we obtain a \(K(A,n)\).

Next aim: for \(X\) an \((n-1)\)\bhyph connected based CW\nbhyph complex,
\begin{equation*}
...
\begin{lemma}[Schwede Lem.~49]
    Let \((X,Y)\) be a relative CW\nbhyph complex and \(Z\) any space.
    Suppose that for all \(m \ge 1\) such that \((X,Y)\) has at least one relative \(m\)\nbhyph cell, \(\pi_{m-1}(Z,z) = 0\) for all \(z \in Z\).
\end{lemma}

\begin{proof}
    By induction on the relative skeleta, we construct continuous maps \(f_m \colon X_m \to Z\) such that \(f_{-1} = f\), \(f_i|_{X_{i-1}} = f_{i-1}\).
    We define \(f_0 \colon X_0 = Y \amalg I \to Z\) by sending the new \(0\)\nbhyph cells arbitrarily to~\(Z\).
    For \(n \ge 1\), if \(X_i \ne X_{i-1}\), then there is at least one relative \(i\)\nbhyph cell.
    The attaching map~\(\alpha_j\) for every relative \(i\)\nbhyph cell becomes null homotopic after composition with \(f_{i-1} \colon X_{i-1} \to Z\)  because \(\pi_{i-1}(Z,*) = 0\).
    We choose a null homotopy \(h_j \colon D^{i+1} \to Z\) of this composite and define \(f_i \colon X_i \to Z\) by \(h_j\) on the \(j\)th \(i\)\nbhyph cell.
    \begin{equation*}
        \begin{tikzcd}
...
\begin{proof}
    ...
    \end{tikzcd}\]
    So
    \[\mathrm{Nat}_{\mathbf{Top}}(H^n(\_,A), H^m(\_, B)) \xleftarrow{\cong} \mathrm{Nat}_{Ho(\mathbf{Top})}(H_n(\_,A), H^m(\_,B)) \xrightarrow{\cong} \mathrm{Nat}_{Ho(\mathbf{CW})}(H^n(\_,A), H^m(\_, B))\]
...
\begin{example}
    \begin{enumerate}
        ...
        \[\Hom_{grp}(A,B)\to \Oper(H^n(\_, A), H^n(\_, B))\cong H^n(K(A,n),B) \xrightarrow[\text{UCT}]{\cong} \Hom(H_n(K(A,n),\iz), B)\cong_{\text{Hurewicz}} \Hom(\pi_n(K(A,n), *), B) \cong_\phi \Hom(A,B)\]
...
\begin{proof}
    \begin{enumerate}
        \item \[\begin{tikzcd}
            \pi_n(X,*) \ar[dd, bend right, "\cong", "\phi"'] \ar[d, "\rho_*"]\ar[r, "\Sigma"] & \pi_{n+1}(\Sigma X, *) \ar[d, "\varepsilon_*"] \\
        ...
        \[\begin{split}
            ...
            &= \iota_{A, n+1} \cap \mathrm{Hur}(\varepsilon\cir (\Sigma f)) \\
